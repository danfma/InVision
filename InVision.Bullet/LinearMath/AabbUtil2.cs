...
namespace InVision.Bullet.LinearMath
{
    public class AabbUtil2
    {
        ...
        {
            if (System.Math.Min(System.Math.Min(vertices[0].X, vertices[1].X), vertices[2].X) > aabbMax.X) return false;
            if (System.Math.Max(System.Math.Max(vertices[0].X, vertices[1].X), vertices[2].X) < aabbMin.X) return false;

            if (System.Math.Min(System.Math.Min(vertices[0].Z, vertices[1].Z), vertices[2].Z) > aabbMax.Z) return false;
            if (System.Math.Max(System.Math.Max(vertices[0].Z, vertices[1].Z), vertices[2].Z) < aabbMin.Z) return false;

            if (System.Math.Min(System.Math.Min(vertices[0].Y, vertices[1].Y), vertices[2].Y) > aabbMax.Y) return false;
            if (System.Math.Max(System.Math.Max(vertices[0].Y, vertices[1].Y), vertices[2].Y) < aabbMin.Y) return false;
            return true;
        }
                    ...
                    for (i = 0; i != 3; ++i)
                    {
                        ...
                        else if ((targetOutcode & bit) != 0)
                        {
                            float lambda = (-MathUtil.VectorComponent(ref source, i) -
                                MathUtil.VectorComponent(ref aabbHalfExtent, i) * normSign) /
                                MathUtil.VectorComponent(ref r, i);
                            lambda_exit = System.Math.Min(lambda_exit, lambda);
                        }
                        bit <<= 1;
                    }
                    normSign = -1f;
