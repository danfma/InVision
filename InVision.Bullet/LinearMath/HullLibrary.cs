	...
	public class HullLibrary
	{
		...
		public static int MaxDirSterId(IList<Vector3> p, int count, ref Vector3 dir, IList<int> allow)
		{
			...
			while (m == -1)
			{
				...
				{
					float s = (float)System.Math.Sin(MathUtil.SIMD_RADS_PER_DEG * (x));
					float c = (float)System.Math.Cos(MathUtil.SIMD_RADS_PER_DEG * (x));
					...
					if (ma != -1 && ma != mb)  // Yuck - this is really ugly
					{
						...
						{
							float s1 = (float)System.Math.Sin(MathUtil.SIMD_RADS_PER_DEG * (xx));
							float c1 = (float)System.Math.Cos(MathUtil.SIMD_RADS_PER_DEG * (xx));
		...
		public void B2bFix(HullTriangle s, HullTriangle t)
		{
			for (int i = 0; i < 3; i++)
			{
				...
				int a = s.At(i1);
				int b = s.At(i2);
				System.Diagnostics.Debug.Assert(m_tris[s.Neib(a, b)].Neib(b, a) == s.id);
				System.Diagnostics.Debug.Assert(m_tris[t.Neib(a, b)].Neib(b, a) == t.id);
		...
		void CheckIt(HullTriangle t)
		{
			...
			System.Diagnostics.Debug.Assert(m_tris[t.id] == t);
			for (int i = 0; i < 3; i++)
			{
				int i1 = (i + 1) % 3;
				int i2 = (i + 2) % 3;
				...
				System.Diagnostics.Debug.Assert(m_tris[t.n.At(i)].Neib(b, a) == t.id);
			}
		}
		...
		{
			System.Diagnostics.Debug.Assert(m_tris[tri.id] == tri);
			m_tris[tri.id] = null;
		}
		...
		public int CalcHullGen(IList<Vector3> verts, int verts_count, int vlimit)
		{
			...
			System.Diagnostics.Debug.Assert(epsilon != 0.0);
			...
			for(int j=0;j<m_tris.Count;j++)
			{
				...
				System.Diagnostics.Debug.Assert(t.vmax < 0);
			...
			while(vlimit >0 && ((te=Extrudable(epsilon)) != null))
			{
				...
				System.Diagnostics.Debug.Assert(isextreme[v] == 0);  // wtf we've already done this vertex
				isextreme[v]=1;
				...
				while(j-- > 0)
				{
					...
					if(Above(verts,nt,center,0.01f*epsilon)  || Vector3.Cross(verts[nt.At(1)]-verts[nt.At(0)],verts[nt.At(2)]-verts[nt.At(1)]).Length()< epsilon*epsilon*0.1f )
					{
						...
						System.Diagnostics.Debug.Assert(nb != null);
						System.Diagnostics.Debug.Assert(!HasVert(nb, v));
						System.Diagnostics.Debug.Assert(nb.id < j);
						Extrude(nb,v);
						j=m_tris.Count; 
					}
				} 
		...
		public void BringOutYourDead(IList<Vector3> verts, int vcount, IList<Vector3> overts, ref int ocount, IList<int> indices, int indexcount)
		{
			...
			for (int i = 0; i < indexcount; i++)
			{
				...
				System.Diagnostics.Debug.Assert(v >= 0 && v < vcount);
				...
				else
				{
					...
					System.Diagnostics.Debug.Assert(ocount >= 0 && ocount <= vcount);
		...
		public static Vector3 ThreePlaneIntersection(Plane p0, Plane p1, Plane p2)
		{
			...
			System.Diagnostics.Debug.Assert(System.Math.Abs(quotient) > 0.000001f);
		...
		public static float DistanceBetweenLines(ref Vector3 ustart, ref Vector3 udir, ref Vector3 vstart, ref Vector3 vdir, ref Vector3? upoint, ref Vector3? vpoint)
		{
			...
			float dist = (float)System.Math.Abs(distu - distv);
			if (upoint.HasValue)
			{
