...
namespace InVision.Bullet.Dynamics.Dynamics
{
	public class RigidBody : CollisionObject
	{
		...
		public Vector3 Gravity
		{
			...
			{
				if (m_inverseMass != 0f)
					m_gravity = value * (1f / m_inverseMass);
				else
					m_gravity = value;
			}
		}
...
#else
			m_linearVelocity *= (float)System.Math.Pow((1f - m_linearDamping), timeStep);
			m_angularVelocity *= (float)System.Math.Pow((1f - m_angularDamping), timeStep);
